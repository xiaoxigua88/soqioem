        ...
        $.dialog({
            ...
            ok: function() {
                ...
                if (fieldEmptyValidate($_importForm)) {
                    ...
                    var count = $("#url").val().trimenter().split("\n").length;
                    var cost = count * jsData.price.DefaultGold;
                    SQ.tips.ask({
                        content: "您当前提交了<b class='text-red'>" + count + "</b>个网址，预计需要消费<b class='text-red'>" + cost + "</b>个优币，是否继续？",
                        title: "网址收录查询提交确认",
                        ok: function() {
                            SQ.post({
    ...
    var createBatchHandle = function(operateName, url, checkType) {
        return function() {
            ...
            SQ.tips.ask({
                ...
                content: "确认" + operateName + (taskCount == 0 || checkType == 3 ? "所有符合条件的" : "选中的 <b class='text-red'>" + taskCount + "</b> 个") + "网址？",
                ok: function() {
                    SQ.post({
