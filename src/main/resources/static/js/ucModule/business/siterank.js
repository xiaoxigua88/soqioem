$(function() {
    ...
    function initMinPage(min, max) {
        var $_minpageSelect = $("#minpage");
        var $_maxpageSelect = $("#maxpage");
        $_minpageSelect.html('<option value="">选择起始页</option>');
        for (var i = 1; i <= 5; i++) {
            $_minpageSelect.append('<option value="' + i + '">第' + i + '页</option>');
        }
        $_minpageSelect.val(min);
        changeMinPage();
        $_maxpageSelect.val(max);
        $_minpageSelect.change(function() { changeMinPage(); });
    };
    function changeMinPage() {
        var $_minpageSelect = $("#minpage");
        var $_maxpageSelect = $("#maxpage");
        $_maxpageSelect.html('<option value="">选择结束页</option>');
        for (var i = $_minpageSelect.val(); i <= 5; i++) {
            $_maxpageSelect.append('<option value="' + i + '">第' + i + '页</option>');
        }
    };
        ...
        $.dialog({
            ...
            ok: function() {
                ...
                if (fieldEmptyValidate($_importForm)) {
                    ...
                    var cost = wordCount * (jsData.price.DefaultGold + (urlCount > 1 ? (urlCount - 1) * jsData.price.ExtendGold : 0));
                    SQ.tips.ask({
                        title: "站点排名挖掘创建确认",
        ...
        SQ.post({
            ...
            success: function(json) {
                ...
                $.dialog({
                    ...
                    init: function() {
                        //初始化页数
                        initMinPage(json.MinPage, json.MaxPage);
                        // 初始化sq-btn-group
                        SQ.component.initTabs();
                        $("#allsnapeBtn input").click(function() {
                            $("#allsnape").val($(this).data("snape_type"));
