        ...
        $.dialog({
            ...
            ok: function() {
                ...
                if (fieldEmptyValidate($_importForm)) {
                    ...
                    SQ.tips.ask({
                        content: "您当前提交了<b class='text-red'>"+count+"</b>个域名，预计需要消费<b class='text-red'>"+cost+"</b>个优币，是否继续？",
                        title: "域名SEO查询提交确认",
                        ok: function() {
                            SQ.post({
	...
	var createBatchHandle = function (operateName, url, checkType) {
		return function () {
			...
			SQ.tips.ask({
				...
				content: "确认" + operateName + (taskCount == 0||checkType == 3 ? "所有符合条件的" : "选中的 <b class='text-red'>" + taskCount + "</b> 个") + "域名？",
				ok: function () {
					SQ.post({
