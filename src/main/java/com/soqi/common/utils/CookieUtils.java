...
public class CookieUtils {
	...
	public static void addCookie(String name, Object object) {
		try {
			...
			String v = URLEncoder.encode(FastJsonUtil.toJSONString(object), "UTF-8");
	...
	public static <T> T getCookie(String name, Class<T> clazz) {
		try {
			...
			if (v != null) {
				return FastJsonUtil.parseObject(v, clazz);
			}
		} catch (Exception e) {
	...
	public static String getCookie(String name) {
		try {
			...
			for (int i = 0; i < (cookies == null ? 0 : cookies.length); i++) {
				if ((name).equalsIgnoreCase(cookies[i].getName())) {
					return URLDecoder.decode(cookies[i].getValue(), "UTF-8").replace("\"", "");
				}
			}
		} catch (Exception e) {
